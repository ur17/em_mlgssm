...
class Test_EM_lgssm(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.input_state_mat = np.asarray([[1, 1]])
    ...
    def test_param_update(self):
        model = EM_lgssm(
            ...
            state_input = False, obs_input = False,
    ...
    def test_param_update_with_input(self):
        model = EM_lgssm(
            time_series = self.time_series, 
            state_dim = self.state_dim, obs_dim = self.obs_dim,
            state_input = True, obs_input = True,
        ...
        obs_mat = model._update_obs_mat(e_zn, e_znzn)
        obs_cov = model._update_obs_cov(e_zn, e_znzn)
        input_state_mat = model._update_input_state_mat(e_zn)
        input_obs_mat = model._update_input_obs_mat(e_zn)
        ...
        test_input_state_mat = _em_transition_offset(
            transition_matrices = self.state_mat.reshape(self.state_dim, self.state_dim), 
            smoothed_state_means = e_zn.reshape(len(self.time_series), self.state_dim)
        ).reshape(self.state_dim, self.obs_dim)
    ...
    def test_em_fit_with_input(self):
        model = EM_lgssm(
            time_series = self.time_series, 
            state_dim = self.state_dim, obs_dim = self.obs_dim,
            state_input = True, obs_input = True,
            input_state_dim = 1, input_obs_dim = 1,
            input_state_series = self.input_state_series, 
            input_obs_series = self.input_obs_series
        )
        ...
        kf = KalmanFilter(
            transition_matrices = self.state_mat, observation_matrices = self.obs_mat,
            transition_covariance = self.state_cov, observation_covariance = self.obs_cov,
            transition_offsets = np.asarray([1., 1.]), observation_offsets = np.asarray([1.]),
            initial_state_mean = self.init_state_mean, initial_state_covariance = self.init_state_cov,
            em_vars = ['transition_matrices', 'observation_matrices', 'transition_covariance', 
            'observation_covariance', 'initial_state_mean','initial_state_covariance'], 
            n_dim_state = self.state_dim, n_dim_obs = self.obs_dim
